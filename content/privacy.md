...
- **Notes sync**, a feature we are preparing that writes your call notes into the Notes column of the spreadsheet you selected. It is off in the current version. The App asks Google for write access only if you turn it on, and when on it writes nothing else and edits no other file.
